<div class="previous-layout">
  <div class="previous-header text-xxs text-upper text-bold colorTextBlackSoft">WED, AUGUST 29, 2018 3:04 PM</div>
  <div class="product-cell jjk c12">
      <div class="title-row c12">
        <div class="product-img">
        </div>
        <div class="row product-title c12 nowrap vcenter">
          <div class="sb-heading sb-heading--small">
            Latte Macchiato
          </div>
        </div>
      </div>
      <div class="detail-row c12">
        <div class="descriptor">Venti
        </div>
      </div>
      <div class="detail-row c12 pt1">
        <div class="descriptor">1 Caramel Syrup pump
        </div>
      </div>
      <div class="detail-row c12 pt2 pb0">
        <div class="descriptor red-text bold">Sold out at this store
        </div>
        <div class="warning">
        </div>
      </div>
      <div class="utility-row c12">
        <div class="utility-actions">
          <div class="heart"><a href="#"></a>
            <div class="heart-hover"></div>
              <div class="heart-img"></div>
          </div>
          <div class="plus"><a href="#"></a>
            <div class="plus-hover"></div>
              <div class="plus-img"></div>
          </div>
        </div>
      </div>
      <div class="c12 pt6 pb4">
        <hr>
      </div>
      <div class="title-row c12">
        <div class="product-img">
        </div>
        <div class="row product-title c12 nowrap vcenter">
          <div class="sb-heading sb-heading--small">
            Latte Macchiato
          </div>
        </div>
      </div>
      <div class="detail-row c12">
        <div class="descriptor">Tall
        </div>
      </div>
      <div class="detail-row c12 pt1">
        <div class="descriptor">2 Vanilla Syrup pumps
        </div>
      </div>
      <div class="detail-row c12 pt2 pb0">
        <div class="descriptor red-text bold">Sold out at this store
        </div>
        <div class="warning">
        </div>
      </div>
      <div class="utility-row c12">
        <div class="utility-actions">
          <div class="heart"><a href="#"></a>
            <div class="heart-hover"></div>
              <div class="heart-img"></div>
          </div>
          <div class="plus"><a href="#"></a>
            <div class="plus-hover"></div>
              <div class="plus-img"></div>
          </div>
        </div>
      </div>
    </div>
    <div class="previous-header text-xxs text-upper text-bold colorTextBlackSoft">WED, AUGUST 29, 2018 3:04 PM</div>
    <div class="product-cell c12">
      <div class="title-row c12">
        <div class="product-img2">
        </div>
        <div class="row product-title c12 nowrap vcenter">
          <div class="sb-heading sb-heading--small">
            Double-Smoked Bacon, Cheddar & Egg Sandwich
          </div>
        </div>
      </div>
      <div class="detail-row c12 pt1">
        <div class="descriptor">1 Sriracha packet
        </div>
      </div>

      <div class="detail-row c12 pt1 pb2">
      </div>
      <div class="utility-row c12">
        <div class="utility-actions">
          <div class="heart"><a href="#"></a>
            <div class="heart-hover"></div>
              <div class="heart-img"></div>
          </div>
          <div class="plus"><a href="#"></a>
            <div class="plus-hover"></div>
              <div class="plus-img"></div>
          </div>
        </div>
      </div>
      <div class="rewards-row c12">
        <div class="birthday-row c12">
          <div class="reward-price">-$3.95</div>
        </div>
      </div>
    </div>
    <div class="previous-header text-xxs text-upper text-bold colorTextBlackSoft">MON, AUGUST 27, 2018 4:21 PM</div>
    <div class="product-cell c12">
      <div class="title-row c12">
        <div class="product-img3">
        </div>
        <div class="row product-title c12 nowrap vcenter">
          <div class="sb-heading sb-heading--small">
            Espresso Roast Verismo® Pods
          </div>
        </div>
      </div>
      <div class="detail-row c12">
        <div class="descriptor">12 count
        </div>
      </div>
      <div class="detail-row c12 pt1 pb2">
      </div>
      <div class="utility-row c12">
        <div class="utility-actions">
          <div class="heart"><a href="#"></a>
            <div class="heart-hover"></div>
              <div class="heart-img"></div>
          </div>
          <div class="plus"><a href="#"></a>
            <div class="plus-hover"></div>
              <div class="plus-img"></div>
          </div>
        </div>
      </div>
      <div class="rewards-row c12">
        <div class="birthday-row c12">
        </div>
      </div>
    </div>
    <div class="previous-header text-xxs text-upper text-bold colorTextBlackSoft">THU, AUGUST 23, 2018 5:01 PM</div>
    <div class="product-cell c12">
      <div class="title-row c12">
        <div class="product-img4">
        </div>
        <div class="row product-title c12 nowrap vcenter">
          <div class="sb-heading sb-heading--small">
            Chai Crème Frappuccino®
          </div>
        </div>
      </div>
      <div class="detail-row c12">
        <div class="descriptor">Grande
        </div>
      </div>
      <div class="detail-row c12 pt1">
        <div class="descriptor">2 shots
        </div>
      </div>
      <div class="detail-row c12 pt1 pb2">
      </div>
      <div class="utility-row c12">
        <div class="utility-actions">
          <div class="heart"><a href="#"></a>
            <div class="heart-hover"></div>
              <div class="heart-img"></div>
          </div>
          <div class="plus"><a href="#"></a>
            <div class="plus-hover"></div>
              <div class="plus-img"></div>
          </div>
        </div>
      </div>
      <div class="rewards-row c12">
        <div class="birthday-row c12">
        </div>
      </div>
    </div>
</div>
